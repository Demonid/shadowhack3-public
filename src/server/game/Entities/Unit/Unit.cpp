...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage *damageInfo, int32 damage, SpellEntry const *spellInfo, WeaponAttackType attackType, bool crit)
{
    ...
    switch (spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
            {
                // Physical Damage
                if (damageSchoolMask & SPELL_SCHOOL_MASK_NORMAL && !(spellInfo->Attributes & SPELL_ATTR0_IMPOSSIBLE_DODGE_PARRY_BLOCK))
                {
                    // Get blocked status
...
SpellMissInfo Unit::SpellHitResult(Unit *pVictim, SpellEntry const *spell, bool CanReflect)
{
    ...
    if (pVictim->GetTypeId() == TYPEID_UNIT && ( pVictim->ToCreature()->IsInEvadeMode() || pVictim->ToCreature()->IsInTimedEvadeMode()) && this != pVictim)
...
bool Unit::HandleProcTriggerSpell(Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch(trigger_spell_id)
    {
        ...
        {
            // NOT FINISHING!
            bool isfinishing = procSpell->AttributesEx & (SPELL_ATTR_EX_REQ_COMBO_POINTS1 | SPELL_ATTR_EX_REQ_COMBO_POINTS2);
