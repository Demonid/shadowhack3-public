        ...
        if (eventId == obj->GetGOInfo()->building.damagedEvent)
        {
            ...
            switch(state->type)
            {
                ...
                case BUILDING_WALL:
                    ...
                    {
                        (*itr)->PlayDirectSound(getDefenderTeam() == TEAM_ALLIANCE ? WG_SOUND_KEEP_ASSAULTED_HORDE : WG_SOUND_KEEP_ASSAULTED_ALLIANCE) ; // Wintergrasp Fortress under Siege
                    }
                ...
                case BUILDING_TOWER:
                    ...
                    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
                    {
                        (*itr)->PlayDirectSound(getDefenderTeam() == TEAM_ALLIANCE ? WG_SOUND_KEEP_CAPTURED_HORDE : WG_SOUND_KEEP_CAPTURED_ALLIANCE) ; // Wintergrasp Fortress under Siege
                    }
        ...
        else if (eventId == obj->GetGOInfo()->building.destroyedEvent)
        {
            ...
            switch(state->type)
            {
                ...
                case BUILDING_TOWER:
                    ...
                    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
                    {
                        (*itr)->PlayDirectSound(getDefenderTeam() == TEAM_ALLIANCE? WG_HORDE_CAPTAIN : WG_ALLIANCE_CAPTAIN) ; // Wintergrasp Fortress under Siege
    ...
    switch(GetCreatureType(entry))
    {
        ...
        case CREATURE_ENGINEER:
            ...
            {                
                ...
                {
                    if ((workshop->GetTeam() == TEAM_ALLIANCE && workshop->m_engGuid_A == creGUIDLow)
                        || (workshop->GetTeam() == TEAM_HORDE && workshop->m_engGuid_H == creGUIDLow)
                        || workshop->GetTeam() == TEAM_NEUTRAL)
                    {
        ...
        case CREATURE_SPIRIT_GUIDE:
            for (OutdoorPvP::OPvPCapturePointMap::iterator itr = m_capturePoints.begin(); itr != m_capturePoints.end(); ++itr)
            {
                if (OPvPCapturePointWG *workshop = dynamic_cast<OPvPCapturePointWG*>(itr->second))
                {
                    if ((workshop->GetTeam() == TEAM_ALLIANCE && workshop->m_spiGuid_A == creGUIDLow)
                        || (workshop->GetTeam() == TEAM_HORDE && workshop->m_spiGuid_H == creGUIDLow)
                        || workshop->GetTeam() == TEAM_NEUTRAL)
                    {
...
void OutdoorPvPWG::HandleKill(Player *killer, Unit *victim)
{
    ...
    {
        if (victim->getLevel() >= WG_MIN_LEVEL)
            ok = true;
    ...
    if (ok)
    {
        ...
        {
            if (killer->getLevel() >= WG_MIN_LEVEL)
            ...
            else if (m_LastResurrectTime > 500)    // Resurrect players only half a second later, to see spirit heal effect on NPC
            {
                for (std::vector<uint64>::const_iterator itr = m_ResurrectQueue.begin(); itr != m_ResurrectQueue.end(); ++itr)
                {
                    if(Player *plr = sObjectMgr->GetPlayer(*itr))
                    {
                        ...
                        plr->CastSpell(plr, 6962, true); //http://info.izbooshka.net/?spell=6962 - wtf?
...
void OutdoorPvPWG::StartBattle()
{
    ...
    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
    {
        ...
        else
        {
            ...
            (*itr)->CastSpell((*itr), SPELL_PARACHUTE_WG, true); //prevent die if fall
            (*itr)->PlayDirectSound(WG_SOUND_START_BATTLE); // START Battle
        }
    }
    ...
    for (PlayerSet::iterator itr = m_players[getAttackerTeam()].begin(); itr != m_players[getAttackerTeam()].end(); ++itr)
    {
        ...
        z = (*itr)->GetBaseMap()->GetHeight(x, y, OutdoorPvPAttackersTower[attackerstowerid][2] + 5.0f);
    ...
    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
    {
        ...
        float x, y, z;
        x = 5330.0f + float(irand(-10, 10));
        y = 2842.0f + float(irand(-10, 10));
        z = (*itr)->GetBaseMap()->GetHeight(x, y, 411.0f);

        (*itr)->TeleportTo(571, x, y, z, 0.0f); 
    }
...
void OutdoorPvPWG::EndBattle()
{
    ...
    for (PlayerSet::iterator itr = m_players[getDefenderTeam()].begin(); itr != m_players[getDefenderTeam()].end(); ++itr)
    {
        (*itr)->PlayDirectSound(getDefenderTeam() == TEAM_ALLIANCE ? WG_SOUND_ALLIANCE_WINS : WG_SOUND_HORDE_WINS) ; // SoundOnEndWin
    }

    for (PlayerSet::iterator itr = m_players[getAttackerTeam()].begin(); itr != m_players[getAttackerTeam()].end(); ++itr)
        (*itr)->PlayDirectSound(WG_SOUND_NEAR_VICTORY) ; // SoundOnEndLoose
