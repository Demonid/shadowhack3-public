...
public:
    ...
    struct boss_azgalorAI : public hyjal_trashAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (RainTimer <= diff)
            {
                DoCast(SelectTarget(SELECT_TARGET_RANDOM,0,30.0f,true), SPELL_RAIN_OF_FIRE);
                RainTimer = 20000+rand()%15000;
            } else RainTimer -= diff;

            if (DoomTimer <= diff)
            {
                DoCast(SelectTarget(SELECT_TARGET_RANDOM,1,100.0f,true), SPELL_DOOM);//never on tank
...
public:
    ...
    struct mob_lesser_doomguardAI : public hyjal_trashAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                DoCast(SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true), SPELL_CRIPPLE);
