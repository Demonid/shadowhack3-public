...
{1973.12f,-90.27f, 330.140f, 0.0f},
{1994.26f,-96.62f, 330.620f, 0.0f},
{2005.41f,-82.88f, 329.500f, 0.0f},
{2042.09f,-41.70f, 329.120f, 0.0f},
{1918.06f, 16.50f, 330.970f, 0.0f},
{1899.59f, -4.87f, 332.137f, 0.0f},
{1897.75f,-48.24f, 332.350f, 0.0f}
};
...
public:
    ...
    {
        boss_saraAI(Creature *pCreature) : BossAI(pCreature, BOSS_MINIBOSS)
        {
//            pInstance = pCreature->GetInstanceData();
            ...
            me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);    //Q: to DB?
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_BLESSING:
                            ...
                            break;
                        case EVENT_ANGER:
                            if (Creature *pGuardian = me->FindNearestCreature(NPC_GUARDIAN_OF_YOGGSARON,50.0f,true))
                ...
                if (uiPhase_timer <= diff)
                {
                    switch (uiStep)
                    {
                        ...
                        case 5:
                            ...
                            DoZoneInCombat();
                            me->GetMotionMaster()->MoveJump(me->GetPositionX(),me->GetPositionY(),me->GetPositionZ()+20, 10.0f, 15.0f);
            ...
            if (insaneTimer <= diff)
            {
                if (instance)
                {
                    ...
                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    {
                        ...
                        if (Creature* pVoice = me->SummonCreature(NPC_YOGG_SARON_VOICE,me->GetPositionX(),me->GetPositionY(),me->GetPositionZ(),0.0f,TEMPSUMMON_TIMED_DESPAWN,1000))
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        case EVENT_TENTACLES:
                            spawnTentacles();
                            events.ScheduleEvent(EVENT_TENTACLES, urand(25*IN_MILLISECONDS, 30*IN_MILLISECONDS), 0, PHASE_2);
                            break;
                        case EVENT_ILLUSION:
                            ...
                            OpenIllusion();
                            events.ScheduleEvent(EVENT_ILLUSION, urand(80*IN_MILLISECONDS, 85*IN_MILLISECONDS), 0, PHASE_2);
                            break;
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_IMMORTAL_GUARDIAN:
                            Position pos;
                            me->GetRandomNearPosition(pos, 25.0f);
                        ...
                        case EVENT_SHADOW_BEACON:
                            if (Creature *pImmortal = me->FindNearestCreature(NPC_IMMORTAL_GUARDIAN,80.0f,true))
                            {
                                Map::PlayerList const &players = instance->instance->GetPlayers();
                                for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
        ...
        void spawnTentacles()
        {
            switch(spawnedTentacles)
            {
                case 0:
                    ...
                    {
                        Position pos;
                        pTarget->GetRandomNearPosition(pos, 5.0f);
...
public:
    ...
    {
        boss_brain_yoggsaronAI(Creature *pCreature) : BossAI(pCreature, BOSS_MINIBOSS)
        {
            me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);    //Q: to DB?
            me->ApplySpellImmune(0, IMMUNITY_ID, 49560, true);  // Death Grip
            me->SetReactState(REACT_PASSIVE);
            ...
            switch (action)
            {
                case ACTION_TENTACLE_COUNT:
                    ...
                    if (tentacleCount == 8)
                    {
                        ...
                        me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        if (GameObject* pDoor = me->FindNearestGameObject(GOB_CHAMBER_DOOR + illusion, 60.0f))
                            pDoor->SetGoState(GO_STATE_ACTIVE);
                        // The Illusion shatters and a path to the central chamber opens!
                        std::list<Unit*> unitList;
                        me->GetRaidMember(unitList, 80.0f);
...
public:
    ...
    struct npc_guardian_yoggsaronAI : public ScriptedAI
    {
        ...
        void JustDied(Unit *victim)
        {
            ...
            if (Creature *pSara = me->FindNearestCreature(NPC_SARA,15.0f,true))
                if (phase == PHASE_1)
...
public:
    ...
    struct npc_death_orbAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (RayTimer <= 0)
            {
                if (Creature* Sara = me->FindNearestCreature(NPC_SARA, 60.0f))
                {
...
class npc_descend_into_madness : public CreatureScript  //Q: WTF? incomplete or to DB entirely?
{
public:
...
class npc_passive_illusions : public CreatureScript //Q: WTF? incomplete or to DB entirely?
{
public:
...
class npc_crusher_tentacle : CreatureScript
{
public:
...
public:
    ...
    struct keeper_imageAI : public ScriptedAI
    {
        ...
        {
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE | UNIT_FLAG_NON_ATTACKABLE);   //Q: to DB?
            me->SetReactState(REACT_PASSIVE);
    ...
    {
        if (pCreature->GetInstanceScript() && pPlayer)
        {
            if (!pCreature->HasAura(SPELL_KEEPER_ACTIVE))
            {
...
public:
    ...
    struct npc_sanity_wellAI : public Scripted_NoMovementAI
    {
        ...
        {
            pInstance = pCreature->GetInstanceScript();
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);    //Q: to DB?
