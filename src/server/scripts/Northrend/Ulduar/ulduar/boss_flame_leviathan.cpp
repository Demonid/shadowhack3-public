...
{
public:
    npc_lorekeeper() : CreatureScript("npc_lorekeeper") { } //DB: changed mob scriptname
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF+2:
            ...
            if (Creature* pLeviathan = instance->instance->GetCreature(instance->GetData64(DATA_LEVIATHAN)))
            {
                ...
                if (Creature* Delorah = pCreature->FindNearestCreature(NPC_DELORAH, 1000.0f, true))
                {
                    if (Creature* Branz = pCreature->FindNearestCreature(NPC_BRANZ_BRONZBEARD, 1000.0f, true))
                    {
    ...
    bool OnTrigger(Player* pPlayer, const AreaTriggerEntry* /*pAt*/)
    {
        if(Creature* vehicle = pPlayer->GetVehicleCreatureBase())
        {
            if (!vehicle->HasAura(SPELL_AUTO_REPAIR) && !pPlayer->isInCombat())
            {
                ...
                pPlayer->CastSpell(vehicle, SPELL_AUTO_REPAIR, true);   //Q: check whether repair spell works
