            ...
            if (uiIntroTimer <= diff)
            {
                ...
                switch (uiIntroPhase)
                {
                    ...
                    case 8:
                        ...
                        break;
                    case 9:
                        if (Creature* pAnnouncertoc5 = me->SummonCreature(CREATURE_ANNOUNCER, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(), TEMPSUMMON_CORPSE_TIMED_DESPAWN, 60000))
                        {
...
public:
    ...
    struct npc_announcer_toc5AI : public ScriptedAI
    {
        ...
        void AggroAllPlayers(Creature* pTemp)
        {
            ...
            if (Creature *pMyVehicle = pTemp->GetVehicleCreatureBase())
            {
                pTemp->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);
                pMyVehicle->SetHomePosition(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation());
                pMyVehicle->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);
                pMyVehicle->SetReactState(REACT_AGGRESSIVE);               
            }
            else
            {
                pTemp->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);
            }
            ...
            for (Map::PlayerList::const_iterator i = PlList.begin(); i != PlList.end(); ++i)
            {
                if (Player* pPlayer = i->getSource())
                {
                    ...
                    {
                        Creature* pCreature = pPlayer->GetVehicleCreatureBase();  
                        ...
                        {                     
                            ...
                            pTemp->AddThreat(pCreature, 0.1f);
                        }
                    }
