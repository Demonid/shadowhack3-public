...
#define GOSSIP_START_EVENT1_RU     "Я готов приступить к испытанию."
#define GOSSIP_START_EVENT2_RU     "Я готов к следующему испытанию."
		...
		void MoveInLineOfSight(Unit* pWho)
		{
			...
			if (Phase == IDLE && pWho->isTargetableForAttack() && me->IsHostileTo(pWho) && me->IsWithinDistInMap(pWho, 20))
			{
				...
				if (pTrall = me->SummonCreature(CREATURE_TRALL, 685.569f, 615.103f, 435.396f, 6.23544f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000))
				{
					...
					pTrall->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
				}
				...
				if (pGarrosh = me->SummonCreature(CREATURE_GARROSH, 685.7f, 621.134f, 435.396f, 6.259f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000))
				{
					...
					pGarrosh->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
				}
				...
				if (pKing = me->SummonCreature(CREATURE_KING, 807.724f, 617.9f, 435.396f, 3.18416f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000))
				{
					...
					pKing->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
				}
				...
				if (pLady = me->SummonCreature(CREATURE_LADY, 807.401f, 613.667f, 435.397f, 3.0585f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000))
				{
					...
					pLady->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
				}
				...
				if (pHighlord = me->SummonCreature(CREATURE_HIGHLORD, 746.482f, 556.857f, 435.396f, 1.5898f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 20000))
				{
					...
					pHighlord->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
				}			
			}
		}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			if (uiIntroTimer <= diff)
			{
				...
				case 9:
					...
					{	
						me->DisappearAndDie();         
						pAnnouncertoc5->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_NPC_FLAG_GOSSIP);
...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pCreature)
	{
		...
		if (pInstance->GetData(BOSS_GRAND_CHAMPIONS) == NOT_STARTED &&
			...
			pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, (pPlayer->isRussianLocale())? GOSSIP_START_EVENT1_RU:GOSSIP_START_EVENT1, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
		else
			pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, (pPlayer->isRussianLocale())? GOSSIP_START_EVENT2_RU:GOSSIP_START_EVENT2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
	...
	struct npc_announcer_toc5AI : public ScriptedAI
	{
		npc_announcer_toc5AI(Creature* pCreature) : ScriptedAI(pCreature)
		{
			...
			me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_NPC_FLAG_GOSSIP);
		...
		void SetData(uint32 uiType, uint32 uiData)
		{
			...
			{
			case DATA_START:
				if (GameObject* pGO = me->GetMap()->GetGameObject(pInstance->GetData64(DATA_MAIN_GATE)))
					pInstance->HandleGameObject(pGO->GetGUID(), true);

				if (GameObject* pGO = me->GetMap()->GetGameObject(pInstance->GetData64(DATA_MAIN_GATE1)))
			...
			case DATA_IN_POSITION: //movement done.		
				...
				me->GetMotionMaster()->MovePoint(1, 735.898f, 651.961f, 411.93f, false);
			...
			case DATA_LESSER_CHAMPIONS_DEFEATED:
				{
					...
					if (uiLesserChampions == 3 || uiLesserChampions == 6)
					{
						...
						for (std::list<uint64>::const_iterator itr = TempList.begin(); itr != TempList.end(); ++itr)
						{
							...
							{
								pSummon->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
								pSummon->SetHomePosition(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation());
		...
		void StartGrandChampionsAttack()
		{
			...
			{
				pGrandChampion1->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
				pGrandChampion1->SetHomePosition(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation());
				pGrandChampion1->SetReactState(REACT_AGGRESSIVE);
				pGrandChampion1->SetInCombatWithZone();

				pGrandChampion2->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
				pGrandChampion2->SetHomePosition(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation());
				pGrandChampion2->SetReactState(REACT_AGGRESSIVE);
				pGrandChampion2->SetInCombatWithZone();

				pGrandChampion3->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE);
				pGrandChampion3->SetHomePosition(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation());
				pGrandChampion3->SetReactState(REACT_AGGRESSIVE);				
				pGrandChampion3->SetInCombatWithZone();
			}
		}
			...
			if (Creature* pBoss = me->SummonCreature(VEHICLE_TO_SUMMON1, SpawnPosition))
			{
				...
				for (uint8 i = 0; i < 3; ++i)
				{
					...
					{			
						...
						{
						case 0:
							pAdd->GetMotionMaster()->MoveFollow(pBoss, 2.0f, M_PI, MOTION_SLOT_ACTIVE, false);
							break;
						case 1:
							pAdd->GetMotionMaster()->MoveFollow(pBoss, 2.0f, M_PI * 0.5f, MOTION_SLOT_ACTIVE, false);
							break;
						case 2:
							pAdd->GetMotionMaster()->MoveFollow(pBoss, 2.0f, M_PI * 0.5f + M_PI, MOTION_SLOT_ACTIVE, false);
							break;
						}
					}
		...
		void StartEncounter()
		{
			...
			if (pInstance->GetData(BOSS_BLACK_KNIGHT) == NOT_STARTED)
			{
				...
				if (pInstance->GetData(BOSS_GRAND_CHAMPIONS) == DONE &&
					(pInstance->GetData(BOSS_ARGENT_CHALLENGE_E) == DONE || pInstance->GetData(BOSS_ARGENT_CHALLENGE_P) == DONE))
				{
					if (Unit* pBlackKnight = me->SummonCreature(VEHICLE_BLACK_KNIGHT, 801.369507f, 640.57428f, 469.314362f, 3.97124f, TEMPSUMMON_DEAD_DESPAWN, 180000))
					{
					...
					me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_NPC_FLAG_GOSSIP);
