...
public:
    ...
    struct boss_kormokAI : public ScriptedAI
    {
        ...
        {
            if (Creature *SummonedMinion = DoSpawnCreature(16119, float(irand(-7,7)), float(irand(-7,7)), 0.0f, 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 120000))
                if (SummonedMinion->AI()) SummonedMinion->AI()->AttackStart(victim);
        }

        void SummonMages(Unit* victim)
        {
            if (Creature *SummonedMage = DoSpawnCreature(16120, float(irand(-9,9)), float(irand(-9,9)), 0.0f, 0.0f, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 120000))
                if (SummonedMage->AI()) SummonedMage->AI()->AttackStart(victim);
        }
